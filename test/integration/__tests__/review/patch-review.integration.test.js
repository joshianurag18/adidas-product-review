...
describe("PATCH review Integration Test", () => {
  ...
  describe("Patch review by product id", () => {
    it("should update review for product id - success case", async () => {
      ...
      const responsePatch = await fastify.inject({
        method: "PATCH",
        url: `/v1/reviews/${randomProductId}`,
        accept: "application/json",
    ...
    it("should return error when product does not exist exist", async () => {
      ...
      expect(responseDelete.statusCode).toBe(500);
    });
  });
});
